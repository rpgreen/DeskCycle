...
void sendOutput()
{
  ...
  case STATE_SESSION:
    ...
    if (millis() - lastTickTime > sessionTimeoutMillis)
    {
      Serial.print('Q');
...
void cycleInterrupt()
{
  ...
  if ((currentTime - lastTickTime) > 100)  //TODO: adjust noise threshold
  {
    tickQueue++;
...
void transitionState()
{
  ...
  case STATE_IDLE_W:
    ...
    {  
      ...
      {
        delay(1000);  //wait for the second char
      }
  ...
  case STATE_INIT1_W:
    ...
    {  
      if (Serial.available() == 0) 
      {
        delay(5000);  //wait for the second char, long delay for db query
      }
      char c2 = (char) Serial.read();
      if (c2 == '0')
      {
  ...
  case STATE_INIT2_W:
    ...
    {  
      if (Serial.available() == 0) 
      {
        delay(5000);  //wait for the second char
      }
      char c2 = (char) Serial.read();
      if (c2 == '0')
      {
